...
namespace GoSweet.Controllers
{
    public class BuyListController : Controller
    {
        ...
        public IActionResult BuyList()
        {
            ...
            var groups = _shopwebContext
                .ProductDatatables
                .GroupJoin(
                    _shopwebContext.ProductPicturetables,
                    pdt => pdt.PNumber,
                    ppt => ppt.PNumber,
                    (pdt, ppt) => new { ProductData = pdt, ProductPic = ppt }
                )
                .ToList();
            if (x != null)
            {
        ...
        public ActionResult BuyList(IFormCollection form)
        {
            ...
            if (form != null)
            {
                ...
                //下單成功後 加入notify通知給廠商
                var getneworder =
                    from o in _shopwebContext.OrderDatatables
        ...
        public async Task<IActionResult> BuyListGroup()
        {
            ...
            var gdtg = _shopwebContext
                .GroupDatatables
                .GroupJoin(
                    _shopwebContext.MemberMembertables,
                    gdt => gdt.GNumber,
                    mmt => mmt.GNumber,
                    (gdt, mmt) => new { GroupDatatable = gdt, MemberMembertable = mmt }
                )
                .Where(x => x.GroupDatatable.PNumber == pid)
                .ToList();

            var x = _shopwebContext
                .MemberMembertables
            ...
            {
                MemberMembertable? updateNewMMT = _shopwebContext
                    .MemberMembertables
                    .FirstOrDefault(
                        qqq =>
                            qqq.MNumber
                            == gdtg[0].MemberMembertable.Select(x => x.MNumber).LastOrDefault()
                    );
                if (updateNewMMT != null)
                {
                    ...
                    int mymnumber = gdtg[0]
                        .MemberMembertable
                        .Select(x => x.MNumber)
                        .FirstOrDefault();
                    var membermax =
                        from m in _shopwebContext.MemberMembertables
                        where m.MNumber == mymnumber && m.GMaxpeople == m.MNowpeople
                        select m;
                    if (
                        (membermax.FirstOrDefault() != null) && (membermax.First().MStatus == false)
                    )
                    {
            ...
            result2.OType = true;
            result2.MNumber = Convert.ToInt32(
                gdtg[0].MemberMembertable.Select(x => x.MNumber).FirstOrDefault()
            );
        ...
        public IActionResult BuyListGp()
        {
            ...
            var xxgp = _shopwebContext
                .GroupDatatables
                .Where(p => p.PNumber == pid)
                .Select(p => p.GPrice)
                .FirstOrDefault();
